...
**Line 2** (black background): model + effort level, optional account profile badge (the `MM` chip above, useful when juggling multiple Claude Code logins), elapsed time, session cost (`· $10.10` above), context window bar, 5h/7d API quota bars with reset times and optional pace arrows, Claude service status icon

All meters are color-coded: green under 50%, gold 50-80%, coral 80%+. The optional cache hit-rate meter inverts this, since a high cache rate is good.
...
**Session cost** - Line 2 shows the running session cost in USD right after the elapsed time (`· $10.10` above), read straight from the `cost.total_cost_usd` field Claude Code already passes in the statusline JSON. No `ccusage`, no transcript parsing, no extra API calls: Claude Code computes it client-side (the docs note it's an estimate that can differ from your actual bill, so on a subscription plan read it as a relative gauge, not a charge). It's shown with two decimals (a real-but-tiny cost reads `$<0.01` instead of a misleading `$0.00`), and a fresh session with no cost yet shows nothing. `STATUSLINE_COST=0` hides it.

**Prompt-cache hit rate (opt-in)** - Enable it with `STATUSLINE_CACHE=1` and line 2 shows a compact `⚡ NN%` right after the context size: the share of the last API call's input tokens served from the prompt cache (`cache_read_input_tokens / (input_tokens + cache_creation_input_tokens + cache_read_input_tokens)`, from `context_window.current_usage`). The color scale is inverted from the other meters, green when most of the context is cached (cheap), coral when it's cold, so a healthy session reads green. It's off by default (which is why it isn't in the screenshot above), and hidden anyway before the first API call and right after a `/compact` (when `current_usage` is null).
