...
{{< callout type="info" >}}
uzi ships both a light and a dark theme, and every screenshot in this post follows yours. Flip the light/dark toggle (top right), or your browser's theme, and they switch with it.
{{< /callout >}}
...
Treat it as **alpha**. Features land often, refactors happen often, and breaking changes are on the table. But it is not a toy: it is stable and it works well day to day, having already completed **over 1,200 runs** and spent **over 19 billion tokens** getting here. The upside of catching it this early is that you can help shape where it goes, so try it, file issues and feature requests, send PRs, and tell me what works and what does not. It is a `helm install` away, or a `docker compose up` on your laptop: [github.com/vtmocanu/uzi](https://github.com/vtmocanu/uzi).
...
*Four tabs above: scroll back up and switch between **What it is**, **Install & configure** for the Kubernetes and local setup, **Features** for the plan gate, the judge, CI auto-fix, PR review rework, and the scheduled jobs (feature bingo included), and **TUI & Mobile** for the terminal dashboard and the phone UI.*
